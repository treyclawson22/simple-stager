...
async function generateSpecialCodes() {
  try {
    console.log('🎯 Generating 10 special referral codes for production...')
    
    // First check if codes already exist
    const existingCodes = await prisma.specialReferralCode.findMany()
    if (existingCodes.length > 0) {
      console.log(`✅ Found ${existingCodes.length} existing codes in production:`)
      existingCodes.forEach((code, index) => {
        console.log(`${index + 1}. ${code.code} (${code.credits} credits) - ${code.usedBy ? 'USED' : 'AVAILABLE'}`)
      })
    ...
    console.log('🔗 Signup URL format: https://simple-stager-web-production.up.railway.app/signup?ref=VIP1234')
