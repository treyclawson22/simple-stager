...
import { TestResultsWithReenhancement } from './test-results-with-reenhancement'
...
interface TestWorkflowCreatorProps {
  userId: string
  userCredits?: number
...
export function TestWorkflowCreator({ userId, userCredits = 0, onCreditsUpdate, resumeWorkflowId }: TestWorkflowCreatorProps) {
  ...
  useEffect(() => {
    if (resumeWorkflowId) {
      const loadWorkflowData = async () => {
        try {
          // Fetch workflow data to check if it has results and get project name
          const response = await fetch(`/api/test/workflows/${resumeWorkflowId}`)
          if (response.ok) {
